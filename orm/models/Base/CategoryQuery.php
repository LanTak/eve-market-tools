 ...
 * @method     ChildCategoryQuery orderByPublished($order = Criteria::ASC) Order by the published column
 *
 ...
 * @method     ChildCategoryQuery groupByPublished() Group by the published column
 *
 ...
 * @method     ChildCategory findOneByPublished(int $published) Return the first ChildCategory filtered by the published column *
 ...
 * @method     ChildCategory requireOneByPublished(int $published) Return the first ChildCategory filtered by the published column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildCategory[]|ObjectCollection findByPublished(int $published) Return ChildCategory objects filtered by the published column
...
abstract class CategoryQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT category_id, name, published FROM category WHERE category_id = :p0';
        try {
    ...
    public function filterByPublished($published = null, $comparison = null)
    {
        if (is_array($published)) {
            ...
            if (isset($published['min'])) {
                $this->addUsingAlias(CategoryTableMap::COL_PUBLISHED, $published['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($published['max'])) {
                $this->addUsingAlias(CategoryTableMap::COL_PUBLISHED, $published['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CategoryTableMap::COL_PUBLISHED, $published, $comparison);
    }
